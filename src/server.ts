...
function sendText(res: ServerResponse, s: string, code = 200, ctype = "text/plain; charset=utf-8"): void {
...
function sendStatic(res: ServerResponse, name: string): void {
  const path = resolve(join(WEB_DIR, name));
  // Path traversal guard: the resolved path must stay inside WEB_DIR.
  if (!path.startsWith(WEB_DIR)) return sendText(res, "Not found", 404);
  try {
